...
6) Quelles sont les extensions possibles de votre modèle ?
    ...
    On peut ajouter la gestion de plusieurs adresses par client en ajoutant une nouvelle table : adresse_client
    avec les champs : id_adresse, id_client (FK), rue, ville, code_postal, est_defaut (boolean), type ('livraison', 'facturation').
    La table commande devra référencer l'id_adresse utilisée pour la livraison au lieu de stocker directement les champs d'adresse.

    On pourra ajouter une nouvelle colonne dans commande : id_adresse_livraison (FK vers adresse_client).

    n peut également gérer l'historique des prix avec une nouvelle table : historique_prix avec les champs : id_historique, id_produit (FK), ancien_prix (NUMERIC), nouveau_prix (NUMERIC), date_debut (TIMESTAMP).
    Cela pourra permettre de tracer quand et comment le prix catalogue d'un produit a évolué.

    On peut gérer les avis clients avec une nouvelle Table : avis_client et les champs : id_avis, id_client (FK), id_produit (FK), note (INTEGER CHECK 1-5), commentaire (TEXT), date_soumission (TIMESTAMP).
    On pourrait donc ajouter une colonne de statistiques dans la table produit pour le score moyen et le nombre d'avis.

    Enfin, on peut mettre des images multiples par produit avec une nouvelle Table : image_produit avec les champs : id_image, id_produit (FK), url_image (VARCHAR), ordre (INTEGER), est_principale (BOOLEAN).

    On pourra supprimer image_url de la table categorie (ou laisser pour l'image d'en-tête de la catégorie, mais utiliser la nouvelle table pour les produits).
